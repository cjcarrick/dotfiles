...
vim.cmd 'inoremap <S-Space> <C-x><C-o>'
...
vim.api.nvim_clear_autocmds { group = 'completion' }
vim.api.nvim_create_autocmd('TextChangedI', {
  group = 'completion',
  callback = function()
    if vim.fn.pumvisible() ~= 0 then return end
    if vim.g.pum_timer then vim.fn.timer_stop(vim.g.pum_timer) end
    if vim.api.nvim_get_current_line():sub(1, vim.api.nvim_win_get_cursor(0)[2]):match '[%w_.@-]$' then
