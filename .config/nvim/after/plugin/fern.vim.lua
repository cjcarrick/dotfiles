...
vim.api.nvim_create_autocmd({ 'FileType' }, {
  pattern = 'fern',
  callback = function()
    local set = function(key, action) vim.keymap.set('n', key, action, { buffer = true }) end
    ...
    set('y', '<Plug>(fern-action-clipboard-copy)')
    set('p', '<Plug>(fern-action-clipboard-paste)')
    set('D', '<Plug>(fern-action-trash)')
    set('o', '<Plug>(fern-action-open:system) <cr>')
